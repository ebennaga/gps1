                    ...
                    for (const j in data[i]) {
                        ...
                        for (const k in data[i][j]) {
                            if (data[i][j][k]['ship_history_ships_latest']) {
                                timeShip = Date.parse(data[i][j][k]['ship_history_ships_latest']['message_utc']);
        ...
        for (let terminalId in locations) {
            ...
            if (message.path) {
                ...
                $.each(histories, function (i, history) {
                    ...
                    let timeShip = Date.parse(history['message_utc']);
                ...
                for (let i in data) {
                    for (const j in data[i]) {
                        if (data[i][j]['ship_history_ships_latest'].length > 0) {
                            let timeShip = Date.parse(data[i][j]['ship_history_ships_latest'][0]['message_utc']);
